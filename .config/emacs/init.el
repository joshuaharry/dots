...
(defun jlib/--random-string (&optional size)
  "Create a random string."
  (let ((alnum "abcdefghijklmnopqrstuvwxyz0123456789")
	(res ""))
    (dotimes (_ (or size 10) res)
      (let ((i (% (abs (random)) (length alnum))))
	(setq res (concat res (substring alnum i (1+ i))))))))

(defun jlib/--parse-home-shell-cmd (cmd)
  "Parse CMD into a list of arguments we can pass to a shell."
  (if (string-match-p "home commit -m " cmd)
      (let ((l (list "home" "commit" "-m")))
	(add-to-list 'l (cadr (split-string cmd "home commit -m ")) t))
    (split-string cmd " ")))

(defun jlib/--shell-and-then (cmd callback)
  "Run CMD in a shell; then, execute the callback function."
  (let ((buf (generate-new-buffer (concat " *" (jlib/--random-string) "*"))))
    (set-process-sentinel
     (apply (apply-partially #'start-process cmd buf) (jlib/--parse-home-shell-cmd cmd))
     (lambda (process signal)
       (kill-buffer buf)
       (funcall callback)))))

(defun jlib/--make-generator (args)
  "Return a function that will traverse further down ARGS every time it is called."
  (let* ((cur args)
	 (arg (car args)))
    (lambda ()
      (if (null cur) nil
...
(defun jlib/--dotfiles ()
  "Get a list of all the dotfiles in ~/dotfiles.dots"
  (let ((dotfiles (jlib/path-join (getenv "HOME") "dotfiles.dots")))
    (when (file-exists-p dotfiles)
      (mapcar
       (lambda (s) (replace-regexp-in-string "~" (getenv "HOME") s))
       (with-temp-buffer
	 (insert-file-contents (jlib/path-join (getenv "HOME") "dotfiles.dots"))
	 (split-string (buffer-string) "\n" t))))))

(defun jlib/--push-home-files (commit-message)
  "Push my dotfiles to GitHub."
  (jlib/--shell-and-then
   (concat "home commit -m " commit-message)
   (lambda ()
     (jlib/--shell-and-then
      "home push origin main"
      (lambda () (message "Dotfiles pushed successfully."))))))

(defun homeadd (&optional and-then)
  "Add all of the files in ~/dotfiles.dots to git; if given AND-THEN, run it."
  (interactive)
  (letrec ((dots (mapcar (lambda (s) (concat "home add " s)) (jlib/--dotfiles)))
	   (gen (jlib/--make-generator dots))
	   (do-next
	    (lambda ()
	      (let ((next (funcall gen)))
		(if next (jlib/--shell-and-then next do-next)
		  (when and-then
		    (funcall and-then)))))))
    (funcall do-next)))

(defun homepush ()
  "Push all of my home files to GitHub."
  (interactive)
  (let ((commit-message (read-string "Enter a commit message: ")))
    (homeadd
     (lambda ()
       (jlib/--push-home-files commit-message)))))
